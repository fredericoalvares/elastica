...
if [ "$CMD" = "out"  ]; then
   ...
   AVAILABLE_VMS=
   for name_vm in `nova list | grep $TIER'_VM' | tr '|' ' ' | tr -s ' ' | cut -d ' ' -f 2`
   do
        if [ "x$(echo $WORKERS | grep $name_vm)" = "x" ]; then
            AVAILABLE_VMS=$AVAILABLE_VMS" "$name_vm	    
	fi 
   done
   avail_vms_arr=( $AVAILABLE_VMS ) 
   for i in `seq 1 $NUMBER_INSTANCES`
   do
       vm_name=${avail_vms_arr[$i]}
  #    vm_name=$TIER'_VM'$(date +"%s")$i
  #    echo "Creating $vm_name / flavor "$FLAVOR_ID
  #    $PROJECT_PATH/apicloud/new_vm.sh $FLAVOR_ID $vm_name lamp $TIER $host #&
       pids[`expr $i - 1`]=$!
       WORKERS=$vm_name' '$WORKERS
   done
   for i in `seq 1 $NUMBER_INSTANCES`
   do
      wait ${pids[`expr $i - 1`]}
   ...
   do
      ...
      ssh -o "StrictHostKeyChecking no" ubuntu@$ip_adress -i $PATH_KEYPAIR/id_rsa "sudo /root/w-setup.sh localhost"

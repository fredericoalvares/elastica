...
PATH_RESULTS=$PROJECT_PATH/experiments/rubis_ctrlf/results
...
#    done
    ...
    do
      ...
      cat /dev/null  > '/tmp/workload_rt.txt'
     
      echo "$(date +%s%N | cut -b1-13) 0 2.0 0.0" > '/tmp/ctrlf_state.log'
...
#		echo "log_cloud_state b_\$1" >> /root/action.sh
	#	echo "$PROJECT_PATH/experiments/rubis_ctrlf/scripts/getCloudState.sh b_\$1 >> $TMP_FILE_LOG_CLOUD_STATE\"_\"\$2" >> /root/action.sh
#                for i in `seq 1 $NUMBER_APPLICATIONS`
#		do
...
#		echo "log_cloud_state e_\$1" >> /root/action.sh
         #       echo "$PROJECT_PATH/experiments/rubis_ctrlf/scripts/getCloudState.sh e_\$1 >> $TMP_FILE_LOG_CLOUD_STATE\"_\"\$2" >> /root/action.sh
                tiers=
		for i in `seq 1 $NUMBER_APPLICATIONS`
		do
...
#state_plateforme.log
		...
		cp /tmp/workload_rt.txt $path_experiment/
		cp /tmp/ctrlf_state.log $path_experiment/
